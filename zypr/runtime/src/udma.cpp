...
UdmaDevice::UdmaDevice(const std::string &devname) : sysfsPath(sysfsRoot + "/" + devname),
...
UdmaRepo::UdmaRepo()
{
  ...
  for (int i = 0;; i++)
  {
    ...
    if (!file_exists(sysfsRoot + "/" + devname))
      break;
...
UdmaDevice *UdmaRepo::device(int index)
{
  ...
  {
    throw std::runtime_error("Attempted to load non-existent buffer");
  }
}
