...
from .zypr_ctrl import ZyprCtrl
...
class Build(Tool):
    def __init__(self, logger, json=None, force=False):
        ...
        self.part = (
            self.config["project"]["project_device"]["device"]
        ...
        self.proxy = self.exists(self.config["config"]["config_xilinx"]["xilinx_proxy"])
        self.petalinux_path = self.config["config"]["config_petalinux"][
            "petalinux_path"
        ]

        # This needs to be added to the config.json
        self.tcl_scripts = []
        self.rtl_hash = self.hash_directory(self.rtl_directory)
        self.no_cores = self.exists(self.config["config"]["config_settings"]["cores"])
    ...
    def setup_vivado_project(self):
        ...
        no_cores = pkg_resources.resource_filename("zypr", f"scripts/vivado")
        ...
        self._renderTemplate(
            jinja_env,
            "no_cores.tcl.j2",
            Path.cwd() / self.work_root / f"{self.project_name}_params.tcl",
            template_vars={
                "cores": self.no_cores if self.no_cores else os.cpu_count() - 1
            },
        )

        colour_logging = pkg_resources.resource_filename(
            "zypr", f"scripts/utils/colours.tcl"
        )
        self.logger.info(colour_logging)

        bd_files = pkg_resources.resource_filename(
            "zypr", f"scripts/vivado/generate_bd_wrapper.tcl"
        )
        self.logger.info(bd_files)

        gen_bd_files = pkg_resources.resource_filename(
            "zypr", f"scripts/vivado/generate_hwdef.tcl"
        )
        self.logger.info(gen_bd_files)

        synth = pkg_resources.resource_filename("zypr", f"scripts/vivado/synth.tcl")
        self.logger.info(synth)

        report_scripts = pkg_resources.resource_filename(
            "zypr", f"scripts/vivado/gen_reports.tcl"
        )
        self.logger.info(report_scripts)

        scripts = pkg_resources.resource_filename("zypr", f"scripts/vivado")
        jinja_env = Environment(loader=FileSystemLoader(scripts))

        self._renderTemplate(
            jinja_env,
            "load_init_prr.tcl.j2",
            Path.cwd() / self.work_root / f"{self.project_name}.inst" / "init_prr.tcl",
            template_vars={
                "configs": list(self.default_configs[self.default_mode].values()),
                "pblocks": (
                    elem[0] for elem in self.default_pblocks[self.default_mode].values()
                ),
            },
        )
        ...
        self._renderTemplate(
            jinja_env,
            "load_prr.tcl.j2",
            Path.cwd() / self.work_root / f"{self.project_name}.inst" / "prr.tcl",
            template_vars={
        ...
        board_files = pkg_resources.resource_filename(
            "zypr", f"boards/{board}/bd/{self.xilinx_version}"
        )
        ...
        checkpoint_path_init = (
            Path.cwd() / self.work_root / f"{self.project_name}.checkpoint" / "init"
        )
        checkpoint_path_final = (
            Path.cwd() / self.work_root / f"{self.project_name}.checkpoint" / "final"
        )
        ...
        generate_sdk_files = {
            ...
            "cmd": [
                f"{self.tool_path}/Vivado/{self.xilinx_version}/bin/vivado",
                "-mode",
                "batch",
                "-source",
                gen_bd_files,
        ...
        for script in self.tcl_scripts:
            self.logger.info(f"Inserting TCL script {script} to project")
    ...
    def run(self):
        ...
        success = False
        click.secho("Starting ZyPR FPGA build...", fg="magenta")
    ...
    def extract(self):
        ...
        for mode in mode_store.keys():
            ...
            for region in mode_store[mode]["pr_regions"].keys():
                self.default_configs[mode][region] = mode_store[mode]["pr_regions"][
                    region
                ]["default_config"]
                self.valid_regions.append(
                    mode_store[mode]["pr_regions"][region]["valid_configs"]
                )
                self.default_pblocks[mode][region] = mode_store[mode]["pr_regions"][
                    region
                ]["pblock"]
                self.logger.info(
                    f"Mode {mode} - {region} - default config: {self.default_configs[mode][region]} - default pblock {self.default_pblocks[mode][region]}"
                )
            modes.add(mode)
            for config in mode_store[mode]["configs"].keys():
                ...
                for module in mode_store[mode]["configs"][config]["modules"]:
                    ...
                    dep_files = set()
                    tcl_files = set()
                    for rtl in mode_store[mode]["configs"][config]["modules"][module][
                        "rtl"
                    ]:
                        if Path(rtl).suffixes == [".tar", ".gz"]:
                            tar = tarfile.open(rtl, "r:gz")
                            tar.extractall()
                            tar.close()
                            p = Path(rtl.cwd()).glob("**/*")
                            files = [x for x in p if x.is_file()]
                            module_files.add(loc)
                        else:
                            loc = str(Path(rtl).parent) + "/" + Path(rtl).stem
                            print(loc)
                            module_files.add(loc)
                    if (
                        "dependencies"
                        in mode_store[mode]["configs"][config]["modules"][module]
                    ):
                        for dep in mode_store[mode]["configs"][config]["modules"][
                            module
                        ]["dependencies"]:
                            dep_files.add(dep)
                        self.modules["modules"][module]["dependencies"] = list(
                            dep_files
                        )
                    if "tcl" in mode_store[mode]["configs"][config]["modules"][module]:
                        for tcl in mode_store[mode]["configs"][config]["modules"][
                            module
                        ]["tcl"]:
                            tcl_files.add(tcl)
                        self.modules["modules"][module]["tcl"] = list(tcl_files)
                    if "ip" in mode_store[mode]["configs"][config]["modules"][module]:
                        for ip in mode_store[mode]["configs"][config]["modules"][
                            module
                        ]["ip"]:
                            loc = str(Path(ip).parent) + "/" + Path(ip).stem
                            ip_files.add(loc)
                        self.modules["modules"][module]["ipcores"] = mode_store[mode][
                            "configs"
                        ][config]["modules"][module]["ip"]
                    self.modules["modules"][module]["rtl"] = list(module_files)
                    self.modules["modules"][module]["top_module"] = mode_store[mode][
                        "configs"
                    ][config]["modules"][module]["top_module"]
                    self.modules["modules"][module]["top_cell"] = mode_store[mode][
                        "configs"
                    ][config]["modules"][module]["top_cell"]
        ...
        if len(ip_files) > 0:
            ...
            if not success:
                self.logger.error(f'Missing module ip {[s + ".xci" for s in ip_files]}')
                exit()
            else:
                self.logger.info(
        ...
        self.flat_protocol_dict = {
            value: key.rsplit("_", 1)[0] for key, value in flat.items()
        }
    ...
    def __ext_modules(self, module, iden):
        ...
        if "ipcores" in self.modules["modules"][module]:
            for each in self.modules["modules"][module]["ipcores"]:
                print(
                    self.modules["ipcores"][
                        str(Path(each).parent) + "/" + Path(each).stem
                    ]
                )
                print(str(Path(each).parent) + "/" + Path(each).stem)
                ip.add(
                    str(
                        self.modules["ipcores"][
                            str(Path(each).parent) + "/" + Path(each).stem
        ...
        self.modules["modules"][module]["obj"] = Module(
            ...
            vhdl=None
            if ("dependencies" not in self.modules["modules"][module])
            else self.modules["modules"][module]["dependencies"],
            ipcores=ip,
            tcl=None
            if ("tcl" not in self.modules["modules"][module])
            else self.modules["modules"][module]["tcl"],
            version=self.xilinx_version,
            force=False,
        )
    ...
    def generate(self):
        ...
        stages = [
            ...
            self.__gen_wrapper(),
            self.__gen_zypr_ctrl(self.config),
    ...
    def __gen_zypr_ctrl(self, config):

        print(self.interfaces)
        z = ZyprCtrl(self.interfaces)

        z.build(f"{Path.cwd() / self.work_root / self.project_name}.inst/zypr.v")

        self.edam["files"].insert(
            1,
            {"name": f"{self.project_name}.inst/zypr.v", "file_type": "verilogSource"},
        )

        return self.verify("Generated ZyPR Controller", True)
    ...
    def __gen_modules(self):
        ...
        for mod in mods:
            ...
            if mods[mod]["config"] in self.default_configs[self.default_mode].values():
                config_defaults[self.modules["modules"][mod]["obj"].name] = mods[mod][
                    "config"
                ]
        ...
        for pr in range(len(self.prr)):
            for mod in mod_list:
                ...
                gen.wrapper(
                    mod, protocol_file=self.flat_protocol_dict, xilinx_pragmas=True
                )
            gen.render(
                f"{self.work_root}/{self.project_name}.inst/",
                build_dir=self.project_name,
                prr=pr,
            )
        ...
        if self.changes == True or (len(checkpoints) != len(mod_list)):
            ...
            self.edam["files"].insert(
                1,
                {
                    "name": f"{self.project_name}.inst/gen_modules.tcl",
                    "file_type": "tclSource",
                },
            )
    ...
    def __gen_base(self, device, custom=None):
        click.secho(f"Generating Base Design for {self.board_name}...", fg="magenta")
    ...
    def __gen_wrapper(self):
        self.edam["files"].insert(
            ...
            {
                "name": str(
                    Path(f"{self.work_root}/{self.project_name}.inst/wrapper.v")
                    .absolute()
                    .relative_to(Path.cwd() / self.work_root)
                ),
        ...
        for each in build_path.glob("blackbox_*"):
            self.logger.info(
                f"Appending blackbox source {str(each.relative_to(Path.cwd() / self.work_root))} to project"
            )
            self.edam["files"].insert(
                1,
                {
                    "name": str(each.relative_to(Path.cwd() / self.work_root)),
                    "file_type": "verilogSource",
                },
            )
    ...
    def __gen_infrastructure(self):
        ...
        ip_config = pkg_resources.resource_filename("zypr", "ip") + "/"
            ...
            for protocol in self.protocol_dict[interface].keys():
                try:
                    ...
                    if Path(f"{ip_config}{interface.lower()}/src").is_dir():
                        for each in Path(f"{ip_config}{interface.lower()}/src").glob(
                            f"{protocol.lower()}*"
                        ):
                            self.edam["files"].insert(
                                1, {"name": str(each), "file_type": "verilogSource"}
                            )
                    interface_len = len(self.protocol_dict[interface][protocol])
                    self.logger.debug(self.protocol_dict[interface][protocol])
                    self.logger.info(
                        f"Found {interface_len} interfaces for {interface} - {protocol}"
                    )
                    self.logger.info(
                        f"Generating IP for {interface_len * len(self.prr)} * {interface} - {protocol} interfaces."
                    )
                    output = open(
                        f"{self.root_path}/.logs/{interface}-{protocol}.log", "w+"
                    )
                    width = 32
                    passthrough = 0
                    axis = (
        ...
        for each in ip_path.glob("*.v"):
            self.logger.info(
                f"Appending IP file {str(each.relative_to(Path.cwd() / self.work_root))} to project"
            )
            self.edam["files"].insert(
                1,
                {
