...
class Build(Tool):
    ...
    def run(self):
        click.secho("Starting ZyPR Linux build...", fg="magenta")
    ...
    def generate(self):
        ...
        bootloader_files = pkg_resources.resource_filename(
            "zypr", f"boards/{board}/linux/{self.xilinx_version}"
        )
        self.logger.info(bootloader_files)

        vitis_scripts = pkg_resources.resource_filename("zypr", f"scripts/vitis")
    ...
    def __prepare_petalinux(self, linux_docker, tool, version):
        success = True
        try:
            self.docker.image.get(f"zypr/{tool}/{self.xilinx_version}")
        except:
            ...
            self.docker_build_image(linux_docker, f"zypr/{tool}/{self.xilinx_version}")
        ...
        try:
            ...
            container = self.docker.containers.run(
                image=f"zypr/{tool}/{self.xilinx_version}",
                stdin_open=True,
                tty=True,
    ...
    def __prepare_kernel(self, tool="petalinux", version="2019.2"):
        ...
        linux_docker = pkg_resources.resource_filename(
            "zypr", f"docker/{tool}/{self.xilinx_version}"
        )
        self.docker_config()
        self.logger.info(f"Using {linux_docker}/Dockerfile to build Linux Kernel")
        try:
            self.logger.info(
                f"Found {self.docker.images.get(f'zypr/{tool}/{self.xilinx_version}').tags}..."
            )
        except:
            self.logger.warning(
                f"Docker image for zypr/{tool}/{self.xilinx_version} not found..."
            )
        if tool == "petalinux":
